...
def create_or_update_periodic_task(
    ...
    **kwargs: Any,
    ...
    task_name = f'Run forwarding: {instance.name} (ID: {instance.id})'
    task_kwargs = {
    ...
    if hasattr(celery_schedule, 'every'):
        # IntervalSchedule
        task_kwargs['interval'] = celery_schedule
        task_kwargs['crontab'] = None
    else:
        # CrontabSchedule
        task_kwargs['crontab'] = celery_schedule
        task_kwargs['interval'] = None
    ...
    if instance.schedule.periodic_task:
        ...
        periodic_task.save()
        logger.info(f'Updated periodic task for {instance.name}')
    else:
        periodic_task = PeriodicTask.objects.create(**task_kwargs)
        instance.schedule.periodic_task = periodic_task
        instance.schedule.save()
        logger.info(f'Created periodic task for {instance.name}')
...
def delete_periodic_task(
    ...
    **kwargs: Any,
    ...
    if instance.schedule and instance.schedule.periodic_task:
        ...
        logger.info(
            f'Deleted periodic task for {instance.name}'
        )

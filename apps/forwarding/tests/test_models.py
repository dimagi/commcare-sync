...
class ForwardingConfigTestCase(TestCase):
    def setUp(self):
...
class ForwardingRunTestCase(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username='testuser', email='test@example.com', password='testpass'
        )
...
class ForwardingDestinationTestCase(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username='testuser', email='test@example.com', password='testpass'
        )
...
class ForwardingSchedulingTestCase(TestCase):
    ...
    def test_creating_config_with_interval_schedule_creates_periodic_task(self):
        """Test that creating a ForwardingConfig with a schedule creates a PeriodicTask."""
        ...
        assert schedule.periodic_task.task == 'apps.forwarding.tasks.run_forwarding_task'
    ...
    def test_creating_config_with_weekly_schedule_creates_periodic_task(self):
        """Test that creating a ForwardingConfig with a weekly schedule creates a PeriodicTask."""
    ...
    def test_creating_config_without_schedule_does_not_create_periodic_task(self):
        """Test that creating a ForwardingConfig without a schedule doesn't create a PeriodicTask."""
        config = ForwardingConfig.objects.create(
    ...
    def test_updating_config_schedule_updates_periodic_task(self):
        """Test that updating a ForwardingConfig's schedule updates the PeriodicTask."""
        from apps.schedules.models import Schedule
    ...
    def test_deleting_config_deletes_periodic_task(self):
        """Test that deleting a ForwardingConfig deletes the PeriodicTask."""
        from apps.schedules.models import Schedule
        from django_celery_beat.models import PeriodicTask
    ...
    def test_is_paused_returns_true_when_no_periodic_task(self):
        """Test that is_paused returns True when schedule has no periodic_task."""
        from apps.schedules.models import Schedule
    ...
    def test_is_paused_returns_false_when_periodic_task_enabled(self):
        """Test that is_paused returns False when periodic_task is enabled."""
        from apps.schedules.models import Schedule
    ...
    def test_is_paused_returns_true_when_periodic_task_disabled(self):
        """Test that is_paused returns True when periodic_task is disabled."""
        from apps.schedules.models import Schedule

...
class TestAdminRequired:
    ...
    def test_authenticated_regular_user_gets_403(self):
        ...
        user = User.objects.create_user(email='regular@example.com', password='testpass')
        request = self._get()
        request.user = user
    ...
    def test_superuser_without_staff_gets_403(self):
        ...
        user = User.objects.create_user(
            email='super_only@example.com', password='testpass', is_superuser=True
        )
        request = self._get()
        request.user = user
        with pytest.raises(PermissionDenied):
    ...
    def test_active_superuser_and_staff_can_access(self):
        ...
        user = User.objects.create_user(
            email='admin@example.com', password='testpass', is_superuser=True, is_staff=True
        )
        request = self._get()
        request.user = user
        response = _protected_view(request)
